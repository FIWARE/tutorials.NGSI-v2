...
with the second. **Authzforce** itself does not offer a UI, and is not concerned with generation and management of XACML
...
`null`. This permission indicates that a user is allowed to make a POST request to the `/door/unlock` endpoint to unlock
the main entrance.
